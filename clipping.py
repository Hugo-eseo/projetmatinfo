...
from shared import (angle_deux_points, determinant_3_points, dist, vabs, sc,
...
def clip(cnv, proj1, proj2, centre, rayon, liste_segments, identite):
    """
    ...
    ecart_proj = vabs(angle_proj2 - angle_proj1)
    ...
    for i in range(nb_seg):
        if A[i] is not None and B[i] is not None:
            milieu_list.append(dist(Point((A[i].x\
                                                  +B[i].x)/2,
    ...
    for i in range(nb_seg):
        if A[i] is not None and B[i] is not None:
            ...
            ecart_AB = vabs(angle_B[i] - angle_A[i])
            if ecart_AB >= 180:
                angle_A[i], angle_B[i] = \
                    angle_B[i], angle_A[i]
                A[i], B[i] = B[i], A[i]
            ...
            if inter1[i] is not None:
                ...
                ecart = \
                    vabs(angle_inter2[i] - angle_inter1[i])
                if ecart >= 180:
                    angle_inter2[i], angle_inter1[i] = \
                        angle_inter1[i], angle_inter2[i]
                    inter1[i], inter2[i] = \
                ...
                if BS < rayon or AS < rayon\
                    ...
                    and point_appartient_segment(inter2[i], segAB)))):
                    ...
                    #quand nécessaire.
                    if vabs(BS-rayon) <= arrondi_segment \
                        and AS > rayon \
                        ...
                        clip = False
                    if vabs(AS-rayon) <= arrondi_segment \
                        and BS > rayon \
                        and intersection_segments(seg22, segAB) is None \
                        and intersection_segments(seg21, segAB) is None\
                        and angle_inter1[i] < angle_proj1\
                    ...
                    if clip:
                        ...
                        #inter1
                        diff = vabs(angle_inter2[i] - \
                                    angle_inter1[i])

                        if diff >= 180:
                            diff = vabs(360 - angle_inter1[i] \
                                        + angle_inter2[i])
                        if diff >= 1:
